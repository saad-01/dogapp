...
import '../view_models/edit_dog_model.dart';

class EditDogPage extends StatelessWidget {
  const EditDogPage({super.key});

  @override
  Widget build(BuildContext context) {
    Color? selectedColor;
    // String selectedOption = '';
    final addDogVM = Get.put(EditDogModel());
    final breedsVM = Get.put(Breeds());
    final formkey = GlobalKey<FormState>();
    final QueryDocumentSnapshot<Object> arguments = Get.arguments;
    final doc = arguments;
    addDogVM.nameController.value = TextEditingController(text: doc['name']);
    addDogVM.weightController.value =
        TextEditingController(text: doc['weight']);
    addDogVM.dateController.value = TextEditingController(text: doc['date']);
    addDogVM.microchipNumberController.value =
        TextEditingController(text: doc['microchipNumber']);
    addDogVM.photoUrl.value = doc['photoUrl'];
    addDogVM.breed.value = doc['breed'];
            ...
            children: [
              ...
              Obx(
                () => PrimartyButton(
                  ...
                  height: 10,
                  onTap: () async {
                    await addDogVM.createDog(doc['dogId']);
                  },
                ),
              ),
              const SizedBox(
